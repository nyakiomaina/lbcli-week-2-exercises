...
BASE_DECODED=$(bitcoin-cli -regtest -rpcport=18332 decoderawtransaction "$BASE_TX")
...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(echo "$BASE_DECODED" | jq -r '.vout[0].value * 100000000 | round')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(echo "$BASE_DECODED" | jq -r '.vout[]
  | select((.value * 100000000 | round) >= 15000000)
  | .n' | head -n 1)
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(echo "$BASE_DECODED" | jq -r --argjson n "$UTXO_VOUT_INDEX" '.vout[]
  | select(.n == $n)
  | (.value * 100000000 | round)')
...
PAYMENT_BTC=$(jq -nr --argjson s "$PAYMENT_AMOUNT" '$s/100000000')
CHANGE_BTC=$(jq -nr --argjson s "$CHANGE_AMOUNT" '$s/100000000')
...
RAW_TX=$(bitcoin-cli -regtest -rpcport=18332 createrawtransaction "$TX_INPUTS" "$TX_OUTPUTS")
...
# WRITE YOUR SOLUTION BELOW:
DECODED_TX=$(bitcoin-cli -regtest -rpcport=18332 decoderawtransaction "$RAW_TX")
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg a "$PAYMENT_ADDRESS" '.vout[]
  | select(.scriptPubKey.address == $a)
  | .value')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg a "$CHANGE_ADDRESS" '.vout[]
  | select(.scriptPubKey.address == $a)
  | .value')
...
SIMPLE_RAW_TX=$(bitcoin-cli -regtest -rpcport=18332 createrawtransaction "$SIMPLE_TX_INPUTS" "$SIMPLE_TX_OUTPUTS")
...
PARENT_TXID=$(bitcoin-cli -regtest -rpcport=18332 decoderawtransaction "$RAW_TX" | jq -r .txid)
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg a "$CHANGE_ADDRESS" '.vout[]
  | select(.scriptPubKey.address == $a)
  | .n')
...
# Convert to BTC
CHILD_SEND_BTC=$(jq -nr --argjson s "$CHILD_SEND_AMOUNT" '$s/100000000')
...
CHILD_RAW_TX=$(bitcoin-cli -regtest -rpcport=18332 createrawtransaction "$CHILD_INPUTS" "$CHILD_OUTPUTS")
...
SECONDARY_DECODED=$(bitcoin-cli -regtest -rpcport=18332 decoderawtransaction "$SECONDARY_TX")
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_VOUT_INDEX=$(echo "$SECONDARY_DECODED" | jq -r '.vout | max_by(.value) | .n')
TIMELOCK_INPUTS='[{"txid":"'"$SECONDARY_TXID"'","vout":'"$TIMELOCK_VOUT_INDEX"',"sequence":10}]'
...
SECONDARY_OUTPUT_VALUE=$(echo "$SECONDARY_DECODED" | jq -r --argjson n "$TIMELOCK_VOUT_INDEX" '.vout[]
  | select(.n == $n)
  | (.value * 100000000 | round)')
...
# Convert to BTC
TIMELOCK_BTC=$(jq -nr --argjson s "$TIMELOCK_AMOUNT" '$s/100000000')
...
TIMELOCK_TX=$(bitcoin-cli -regtest -rpcport=18332 createrawtransaction "$TIMELOCK_INPUTS" "$TIMELOCK_OUTPUTS")
