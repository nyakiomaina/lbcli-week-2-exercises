...
# vout 0 has ~16,430,198 sat which covers 15,000,000
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=0
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(echo "$DECODED_BASE" | jq '(.vout[0].value * 100000000) | round')
...
# TX_SIZE = 10 (base) + 68 * 1 (input) + 31 * 2 (outputs) = 140 vbytes
...
# RBF enabled with sequence 4294967293 (0xFFFFFFFD)
...
# RBF is enabled when sequence < 0xFFFFFFFE (4294967294)
...
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq --arg addr "$CHANGE_ADDRESS" '.vout | to_entries[] | select(.value.scriptPubKey.address == $addr) | .key')
...
# For CSV (BIP 68): nSequence = 10 encodes a 10-block relative lock
...
# vout 1 of SECONDARY_TX = 40,648 satoshis
